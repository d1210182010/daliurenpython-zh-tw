    ...
    def __init__(self, parent=None, shiPan=None):
        ...
        for key in shenShaJson:
            ...
            for sh in shenShaJson[key]:
                if s0 is None:
                    s0 = '<span>{}: {} </span>'.format(cc.convert(sh), shenShaJson[key][sh])
                else:
                    if count % 8 == 0:
                        s0 = '{}<br/><span>{}: {} </span>'.format(s0, cc.convert(sh), shenShaJson[key][sh])
                    else:
                        s0 = '{}&nbsp;&nbsp;&nbsp;&nbsp;<span>{}: {} </span>'.format(s0, cc.convert(sh), shenShaJson[key][sh])
#                 if count == 7:
#                     s0 = "{}<br/>".format(s0)
                count = count + 1
